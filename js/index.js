import { getDocIdByPartnerName, getDocByID, getStuff, showModal, colRef } from "./firestore.js";
...
function panLocation(name) {
  getDocIdByPartnerName(name).then((docId) => {
    getDocByID(docId).then((doc) => {
      ...
      map.flyTo(new L.LatLng(doc.location.latitude, doc.location.longitude));
...
function markerPlacements(partner) {
  var marker = L.marker([
    parseFloat(partner.location.latitude),
    parseFloat(partner.location.longitude),
  ]);
  var popupContent = `
  <div class="leaflet-popup-container">
    <h2 class="partner-header">${partner.partnerName}</h2>          
    ...
    <div class="leaflet-buttons">
      ...
      <div class="deleteButton" data-loc="${partner.partnerName}">Delete<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <div class="editButton" data-loc="${partner.partnerName}">Edit<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none">
  ...
  marker.on("click", function () {
    ...
    for (var i = 0; i < editButtons.length; i++) {
      editButtons[i].addEventListener("click", function () {
        ...
        window.open(
          `editloc.html?partnerName=${encodeURIComponent(partner.partnerName)}`,
          "_blank"
      );
    });
  }
...
// Main stuff
getStuff(partnersArray).then((data_set) => {
  for (var partner = 0; partner < data_set.length; partner++) {
    markerPlacements(data_set[partner])
  }  
})
