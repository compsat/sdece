...
document.addEventListener("click", function (event) {
  if (event.target.matches(".addButton")) {
    ...
    var targetSrc =
      target === "evac" ? "html/addevac.html" : "html/addloc.html";
...
function populateNavBar(condition) {
  ...
  filtered_partners.forEach((partner) => {
    ...
    editBtn.style.cssText =
      "position:absolute;top:8px;right:8px;width:28px;height:28px;border:none;background:transparent;border-radius:6px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#6b7280;padding:0;opacity:0;transition:opacity 0.15s ease,background-color 0.15s ease,color 0.15s ease;";
...
async function onPinClick(doc) {
  ...
  wrapper.querySelectorAll("[data-bind]").forEach((ul) => {
    ...
    switch (key) {
      ...
      case "earthquake_risk_description":
        ul.textContent =
          doc.earthquake_risk_description || "No description found";
        break;
      case "fire_risk":
      ...
      case "landslide_risk_description":
        ul.textContent =
          doc.landslide_risk_description || "No description found";
        break;
      case "storm_risk":
  ...
  wrapper.querySelectorAll(".risk-card").forEach((card) => {
    card.style.display =
      card.getAttribute("data-risk-card") === activeRiskType ? "" : "none";
  });
...
document
  ...
  .addEventListener("click", async () => {
    ...
    for (const riskType of riskTypes) {
      ...
      const households = [...allHouseholds].sort((a, b) =>
        sortByHouseholdName(a, b),
      );
...
function attachMarkers(partners) {
  ...
  partners.forEach((partner) => {
    ...
    marker.on("popupopen", () => {
      setTimeout(() => {
        const edit_button = document.querySelectorAll(
          ".popup-edit-btn:not(.popup-delete-btn)",
        );
        ...
        if (delete_button) {
          delete_button.addEventListener("click", async function () {
            if (
              !confirm(
                `Delete "${partner.household_name}"? This cannot be undone.`,
              )
            )
              return;
            const docId = await getDocIdByPartnerName(partner.household_name);
...
function addEvacCenters() {
  evacCenters.forEach((center) => {
    if (
      appliedShelterTypes.length > 0 &&
      !appliedShelterTypes.includes(center.type)
    ) {
      center.marker = null;
      return;
    }
    ...
    marker.on("popupopen", () => {
      setTimeout(() => {
        const editBtn = document.querySelector(
          `.evac-edit-btn[data-evac-id="${center.id}"]`,
        );
        const deleteBtn = document.querySelector(
          `.evac-delete-btn[data-evac-id="${center.id}"]`,
        );
...
async function applyFilterAndUpdate() {
  ...
  if (appliedShelterTypes.length > 0) {
    const shownCenters = evacCenters.filter((c) =>
      appliedShelterTypes.includes(c.type),
    );
...
export async function presentFilteredData() {
  ...
  const standardMaterialsSelected = rawMaterials.filter(
    (m) => m !== "__other__",
  );
  ...
  if (needsAnyRiskFilter) {
    ...
    baseData.forEach((doc, id) => {
      const matchesRisk = riskFields.some((field) =>
        riskLevels.includes(doc[field]),
      );
...
window.filterMarkersBySearch = function (query) {
  ...
  partnersArray.forEach((partner) => {
    ...
    const addressMatch = regex
      ? regex.test(partner.household_address || "")
      : true;
